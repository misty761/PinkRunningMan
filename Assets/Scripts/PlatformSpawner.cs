...
public class PlatformSpawner : MonoBehaviour {
    ...
    int count = 3; // 생성할 발판의 개수

    float timeBetSpawnMin = 2.25f; // 다음 배치까지의 시간 간격 최솟값
    float timeBetSpawnMax = 3.25f; // 다음 배치까지의 시간 간격 최댓값
    float timeBetSpawn; // 다음 배치까지의 시간 간격

    float yMin = -3.5f; // 배치할 위치의 최소 y값
    ...
    void Start() {
        // count 만큼의 공간을 가지는 새올운 발판 배열 생성
    ...
    void Update() {
        ...
        if (Time.time >= lastSpawnTime + timeBetSpawn)
        {
            ...
            // 사용할 현재 순번의 발판 게임 오브젝트를 비활성화하고 즉시 다시 활성화(왜?)
